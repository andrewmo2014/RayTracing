...
Pipe* SceneParser::parsePipe() {
    ...
    getToken(token); assert (!strcmp(token, "innerR"));
    float innerR = readFloat();
    getToken(token); assert (!strcmp(token, "outerR"));
    float outerR = readFloat();
    getToken(token); assert (!strcmp(token, "}"));
    assert (current_material != NULL);
    return new Pipe(center,height,innerR,outerR,current_material);
}
